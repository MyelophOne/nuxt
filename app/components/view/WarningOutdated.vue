<template>
	<div
		v-if="showWarning"
		class="fixed inset-0 bg-gray-50 flex flex-col items-center justify-center z-50 p-4"
	>
		<div
			class="bg-white shadow-lg rounded-xl p-6 max-w-md w-full text-center flex flex-col items-center"
		>
			<svg
			...
			<h2 class="text-2xl font-semibold mb-2 text-gray-800">
				{{ t("outdated.browserNotSupported") }}
			</h2>
			<p class="text-gray-600 mb-6">
				{{ t("outdated.useModernBrowser") }}:
			</p>
			<div class="flex flex-wrap justify-center gap-4 mb-4">
				...
				>
					<img
						...
						class="w-12 h-12"
					/>
					<span class="text-gray-700 text-sm">Chrome</span>
				</a>
				<a
				...
				>
					<img
						...
						class="w-12 h-12"
					/>
					<span class="text-gray-700 text-sm">Edge</span>
				</a>
				<a
				...
				>
					<img
						...
						class="w-12 h-12"
					/>
					<span class="text-gray-700 text-sm">Firefox</span>
				</a>
			</div>
			<button
				@click="dismiss"
...
const dismissedKey = "browser-warning-dismissed";

const isBrowserUnsupported = () => {
	if (!import.meta.client) return false;

	if (window.location.search.includes("testBrowserWarning")) return true;

	const ua = navigator.userAgent;
	const supported = /(Chrome|Edg|Firefox|Safari)/i;
	return !supported.test(ua);
};
...
onMounted(() => {
	...
	if (isBrowserUnsupported() && !dismissed) {
		showWarning.value = true;
	}
});

const { t } = useMultiLang(["outdated"]);
</script>
