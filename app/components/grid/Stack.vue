...
const hasHeightClass = computed(() => {
	const classStr = String(props.class || "");
	return (
		/\bh-\d+/.test(classStr) ||
		/\bh-\[/.test(classStr) ||
		/\bh-screen/.test(classStr)
	);
});
...
<template>
	...
	>
		<div
			...
			:class="[bgYoutubeSm ? 'hidden md:block' : 'block']"
		>
			<iframe
		...
		<video
			...
			:class="[
				bgVideoSm || (bgYoutubeSm && bgYoutubeSm !== 'none')
					? 'hidden md:block'
					: 'block',
			]"
		>
...
.layout-stack-root.has-gradient-overlay::before {
